...
<!-- sawtracker:comment {"author":"agent:claude-sl211","body_bytes":2477,"created":"1789152317","id":"c1"} -->
...
<!-- sawtracker:comment {"author":"agent:claude-sawlang","body_bytes":445,"created":"1789153307","id":"c2"} -->
RULING RECORDED (user, Sep 11): the auto-wrap is TRANSPARENT to the ownership checkpoint — approved on the design-269 brief's argument (the alternative leaves W1-W5 double-freeing; deletes the one-? spelling divergence; one-line corpus migration). Consequence: an OWNED source forwarded into an Optional/Result auto-wrap demands , matching the unwrapped form. Ships in SL-211.p1 (currently in a review fix round on an unrelated catch-path P1).

<!-- sawtracker:comment {"author":"agent:claude-sawlang","body_bytes":208,"created":"1789153330","id":"c3"} -->
Correction to the previous comment (a shell ate the keyword): the consequence sentence should read — an OWNED source forwarded into an Optional/Result auto-wrap demands `move`, matching the unwrapped form.


<!-- sawtracker:comment {"author":"agent:claude-sawlang","body_bytes":439,"created":"1789159195","id":"c4"} -->
LANDED: closed by SL-211.p1 (merge 1a1ca5c3, Sep 11; design 269). The auto-wrap is TRANSPARENT to the ownership checkpoint (user-ruled): REWRAPS peel at the checkpoint so the wrapped value's transfer is the one judged — the W1-W5 aliasing double-frees are refusals/retains now, and an owned source demands move like its unwrapped neighbour. Pin wrapped_return_takes_the_transfer_checkpoint.saw flipped to passing; V88 carries the matrix.
