...
the codegen release); and V77-V81, DF-299b's value-branch arms — the refusal,
its position-blindness, the six constructs that hand a block tail on, the
...
| V64 | a cast AT THE SAME TYPE forwards the operand's storage, so it takes the move checkpoint | `V64_forwarding_cast_takes_the_move_checkpoint.saw` | DF-299a (the parser census carried it as N10) — DF-216a's named mechanism with `CastExpr` as another member. The checkpoint always RAN over the cast; what it could not do was see the node as a read out of existing storage, because `_is_aliasing_expr` is a node-type test over four classes and `CastExpr` is in none of them. The fix makes the question transparent through a forwarding cast, which is the rule `!` has had since design 131 — `o!` aliases iff `o` does |
| V65 | …and the rule is TIER-BLIND: the Copy tier RETAINS through the cast | `V65_forwarding_cast_is_tier_blind.saw` | DF-299a — V62's lesson at the new position. Fencing only the owning tiers would have left the worse half open: a Copy-tier value came out of the cast UNRETAINED while the source kept its reference, so `strong_count()` read 1 where the plain `let` read 2 and holding both to scope end aborted with `over-release of an Arc (refcount underflow)`. The plain-`let` control beside it is the point — the cast must cost exactly what the read costs |
| V66 | the design-63 distinct-alias projections forward too, in BOTH spellings | `V66_alias_projection_cast_forwards.saw` | DF-299a's obligation-4 sweep, which is what widened the finding from one arm to three. The filing named only the struct identity; the FULL projection (`type Handle = Res`) and the PARTIAL one (`type Outer = Inner`, its own early return a branch above) forward as well. The partial arm was the worse reading — the value was never released at all, where the full one released once for two owners |
| V67 | control: a cast that BUILDS a new value aliases nothing and is untouched | `V67_building_casts_are_untouched.saw` | DF-299a — the line the sweep drew is "does this arm hand back the OPERAND'S storage, or produce a fresh value?", and the building arms produce one: a converted integer, a raw enum tag, a reinterpreted address. The pointer rows matter most, since `(&x) as UnsafePointer<T>` is the only address-of the language has (DF-163f) and the chained `as Int` is the idiom std and the runtime are written in — a forwarding rule that caught them would have broken the runtime seam |
| V68 | a closure's `&`/`&var` PARAMETER names storage the caller owns and may not be `move`d out | `V68_closure_reference_parameter_is_a_borrow.saw` | DF-290a — DF-288a's mechanism at a different registration site. `_check_closure` binds a reference parameter with the REFERENT's type (that is what makes `&var e` read and write the element directly), so the binding's type no longer says it is an alias and the `TypeKind.REFERENCE` test cannot see it: `{ &var e in move e }` saw an owned local, licensed the transfer, and the vector kept the element — `got 7 / drop 7 / drop 7`, exit 0. Both sigils, because the SHARED one is theft through a read-only borrow, exactly the asymmetry V60 records for the match funnel. `with_ref`/`with_var_ref` are std's own scoped-borrow surface, which is why this face owed its own row |
...
| V73 | a by-value closure parameter is OWNED by the body and released exactly once at body end, whether or not the body names it | `V73_closure_by_value_parameter_is_owned.saw` | Design 264 ruling A, closing DF-299c. The by-value arm of closure parameter setup stored the argument and bound the name, and nothing registered it for scope cleanup — the neighbouring comment stated the very rule that was missing ("a deferred-move capture becomes an OWNED local of the body, so the body owes it a scope"), and a by-value parameter is one on identical terms. A named function's parameters DO get released (`methods.py`'s `_needs_cleanup` -> `_register_cleanup`), which is the control that isolated the closure. The body-never-mentions-it face is counted rather than assumed: the leak was in the parameter SETUP, so a rule releasing only what the body touched would have left it open |
| V74 | …and a parameter MOVED into a new owner transfers instead, still exactly one release | `V74_closure_parameter_moved_into_a_new_owner.saw` | Design 264 ruling A. The regression guard, not the fix: this is the one shape that already transferred correctly (DF-299c's edge facts), so the new release must not turn it into a double free. The move clears the scope's drop flag exactly as it does for a DF-218h deferred-move capture. Both exits are checked because they leave the body by different paths — the RETURN and a container the body pushes into |
| V75 | the escaping and non-escaping faces release a by-value parameter identically | `V75_escaping_and_non_escaping_release_identically.saw` | Design 264 ruling A. Where a closure's environment lives is a fact about its CAPTURES; a by-value parameter is handed over at the call either way. DF-299c measured both faces leaking identically, so a fix reaching only the non-escaping one — the shape every std visitor has — would have looked complete against the whole visitor surface and left the other open. The second invocation of one bound closure is here too: the scope is per invocation, not per closure |
| V76 | `Vector.fold` threads its accumulator — N elements, one final owner, one release | `V76_fold_threads_its_accumulator.saw` | Design 264. `fold` is the one std visitor that KEEPS a by-value slot, and this records why that is right: the accumulator is moved in and moved back out once per element, so ruling A's release is what makes it correct. It is also the row the leak was FOUND on (DF-291b, refiled as DF-299c) — a three-element fold over an `Arc` read 1 -> 5, the arithmetic closing at 1 + 1 + 3, and now reads 1 -> 2 with the deinit running. A trivial accumulator registers no cleanup at all, which is the zero-cost half |
| V77 | a value `if`/`match` ARM hands its value to the branch's merged home, so the arm takes the transfer checkpoint | `V77_value_branch_arm_takes_the_move_checkpoint.saw` | DF-299b. `let b = if true { a } else { a }` compiled with no `move` and no `.copy()`: three names for one value, TWO deinits, `a` never retired. The mechanism is a checkpoint never CALLED — which is what separates it from DF-299a next door, a checkpoint that RAN and could not see the node. A branch is not a value; it hands ON one of several block tails, so the transfer is really one PER ARM. Design 195 rule 2 had already said arms are transfers and applied that only to integer widening |
| V78 | …and the refusal reaches every position a value branch can sit in | `V78_value_arm_refusal_is_position_blind.saw` | DF-299b's obligation-4 sweep, first axis. Nine positions, one function each — `let`, argument, `return`, tail, struct field, Vector element, enum payload, assignment RHS, tuple element. The fix is at the CHECKPOINT rather than at the branch, so one edit covers every site that already funnels through it. The argument face was the worst reading of the family: the callee owns its by-value parameter and released it at return, so the caller's still-live binding read freed storage on the next line |
| V79 | every construct that hands a BLOCK TAIL on is a transfer site: `if`, `if let`, `match`, an inline `catch`, a `try { } catch { }` block, and `break <value>` | `V79_every_value_branch_construct_is_a_transfer.saw` | DF-299b's sweep, second axis — six constructs, all open. `break <value>` is the one the transparency funnel cannot reach (its home is the LOOP's merged value, so it takes the checkpoint at the `break` itself) and the only face that ABORTED rather than leaking: `let b = while { break a }` gave two names one value and both ran `deinit`. A `catch` block's tail is an arm exactly as an `else` block's is; nothing about the error path earns a different rule |
| V80 | the Copy tier through a value arm retains EXACTLY ONCE — before the fix and after it | `V80_copy_tier_arm_retains_exactly_once.saw` | DF-299b, and the row that inverts V62/V65 rather than repeating them. Those fences were tier-blind because the compiler handed out an unretained alias; here codegen's `_gen_transfer_value` already names "an inner-block tail expression" as a site the checkpoint does not mark and re-derives the retain, so the Copy tier was CORRECT while the owning tiers aliased — which is why the filing's severity read as a lost deinit, not a double free. So this is a regression guard: stamping the arm must not add a second retain beside the derived one. The PLACE arm is counted because that read carries its own design-146 mark, and two marks on one transfer is where a double retain would show first |
| V81 | control: `move` arms, fresh temporaries, mixed arms, `.copy()` arms and a GENERIC body's arms are untouched | `V81_value_arm_fence_is_narrow.saw` | DF-299b — the fence is correct only if it is narrow. The generic row is the one that could have gone wrong: design 219 wave C counts PER PATH, so `if flag { a } else { b }` names each parameter once on its own path and duplicates neither, and the body stays callable at a move-only type. A per-MENTION reading would have made the commonest generic shape in the language require `Copy` |
| V82 | a closure body's TAIL hands its value to the caller, so it takes the transfer checkpoint its own `return` takes | `V82_closure_tail_takes_the_transfer_checkpoint.saw` | DF-304a. `_check_closure` reconciled the tail's TYPE and checkpointed only the CAPTURES, so nothing judged the tail at all while `return r` one keyword away was refused — one closure, two spellings of one transfer, two answers. A MISSING CALL, which is what separates it from DF-299b next door: that fix made the checkpoint transparent through a value BRANCH, and a branch inside a tail nothing checkpointed is still unchecked |
...
| P17 | a window body reaches the ENCLOSING scope by borrow, not by copy — six spellings, plus the one name deliberately left a value capture | `place_window_borrows_enclosing_locals.saw` | DF-169h — the window closure is a lowering device, so its body is code from the enclosing scope and must run against the live bindings there. It captured by VALUE, which put the copy tier in front of code that copies nothing (`v[i].serialize(to: &var enc)` over a local encoder; `v[0] = w[1]` over two disjoint containers). The row also pins the exclusions: a REFERENCE-typed binding stays a plain capture (its pointer copy is already a borrow), and so does the receiver's own ROOT. A name the body MOVES was a third exclusion until DF-218h; it is now spelled `[move x]` and takes V48's deferred transfer |
| P18 | the window's own ROOT is not borrowed from inside the extent, so `v.push`/`v.pop` there stays refused | `errors/place_window_beside_var_root.saw` (X15) | DF-169h's exclusion half, and why it is an exclusion: borrowing the root would put a second access to it inside the open window. DF-248a settled the boundary: no carve-out to the Law — the assignment face is legalized by ORDER (P21), every other in-window naming keeps this refusal |
| P21 | an assignment's RHS may read the target's own root; every other in-window naming of it is refused, and the refusal teaches which is which | `place_window_root_read_in_assignment_rhs.saw` + `errors/place_window_root_read_teaches_the_asymmetry.saw` | DF-248a — RULED: NO carve-out to the Law. A right-hand side is defined to run before its target (design 193), so lifting a root-naming RHS out of the window REMOVES the overlap rather than permitting one, and DF-218j's hoist widens from a window-opening RHS to any read of the root. Nowhere else has an order to lift along: an argument and a body read run after the accessor's PROLOGUE. The two shapes are indistinguishable on the page, so the refusal names the asymmetry and gives the `let` — that teaching text is the row's other half |
...
| P22 | a std read accessor hands out a place, not a copy it cannot make: `Box<T, A>.value()` lends at EVERY payload tier | `box_value_lends_the_payload.saw` + `errors/box_value_move_only_read.saw` | design 218 unit 1.5 stage 3c, on the Sep-1 ruling. The accessor was bounded `T: ExplicitCopy` with a body spelling `self.ptr[0].copy()`, and the bound did not fence the ERASED box: `Box<any Error>` satisfied it and the substituted body asked an existential for a method its trait does not declare. Invisible while an instance's diagnostics were deleted; the instance check reported it the moment stage 2 made them real, which is what a per-instance judgment is for. A place needs no bound, so the fix removes the family rather than the instance: the Copy tier reads out exactly as before, and a move-only payload gets the window plus the ordinary place-read refusal |
...
| K73 | a driven arm that DISCARDS its payload releases it at extraction, as the sync twin does | `coro_discarded_match_payload_released_at_extraction.saw` + `coro_mixed_match_payload_released_at_extraction.saw` | DF-218w — the last of DF-217p's cells, and the one no scope exit could reach: a `_` binding names no value, so it is registered in no cleanup scope. The first answer was a per-ARM edge (an arm claiming nothing of the scrutinee released the hoisted temp at its start), which could not reach the MIXED `case Both(v, _)` — a named binding aliases the payload for the arm's length, so an arm-start release would free what it reads — and that residue sat XFAIL beside it. Design 247 replaced the edge with the encoding: the temp is take-read, so the scrutinee is an owned temporary and codegen's consume model drops an unclaimed payload inline at extraction and registers a named binding in the arm scope. The residue flipped with it, and both shapes are now decided by the rule the sync twin uses |
...
| K78 | both DISCARD spellings of a singleton spawn form are refused | `K78_singleton_spawn_discard_is_refused.saw` | 242 ruling 5 — the bare statement is the fire-and-forget guess a reader reaches for first, and it used to compile into a sequential call plus thread overhead because the handle's `deinit` joined. `let _ =` is banned here too, which is the one place design 151's blessed explicit discard does not apply: dropping the handle is not a fate the language offers |
| K79 | a BOUND handle that reaches its scope's exit unconsumed is refused, and a consume on ONE path does not discharge it | `K79_unconsumed_thread_handle_is_refused.saw` | 242 ruling 5 — the per-path half, and design 189's join-release machinery read over the consumption question instead of the borrow one. A `join()` inside an `if` is undone on the way out of that block, because the other path never joined |
...
Claim source: spec 8 *Visibility* + *The prelude*; designs 80, 82, 142, 188 u7, 204, 254, 258

| Row | Checks | Covered by | Ruling |
|-----|--------|------------|--------|
| B01 | reading a field marked `private` cross-module | `vis80_field_read_error.saw` | 258 — the field the row reads carried no marker until then, and a bare field is no longer the private one: it inherits its type's tier. The refusal is now pinned on the spelling that MEANS private |
| B02 | writing a field marked `private` cross-module | `vis80_field_write_error.saw` | 258 — same reason as B01: the write half of one gate, re-pinned on the `private` marker |
| B03 | cross-module memberwise literal naming a field marked `private` | `vis80_field_literal_error.saw` | 258 — same reason as B01. Construction visibility IS field visibility, so the literal follows the marker rather than the absence of one |
| B04 | calling a private extension method cross-module | `vis80_method_private_error.saw` | 258 — UNCHANGED on purpose, and the fence on B27: an extension member is a declaration in its own right, so a bare one stays module-private whatever its receiver's tier is |
...
| B07 | control: the public surface IS reachable cross-module — a marked `public` field, a public method, a public static factory, a public init, a conformance method, and (since 258) a BARE field of the same public struct | `vis80_public_members_ok.saw` | 258 — the control gained the inheritance half, because the row's job is to say what a caller CAN reach and a bare field is now on that list |
...
| B23 | a conformance declared under the ORPHAN RULE is coherent PROGRAM-WIDE — carried by every import form, including a `UnsafeSend`/`UnsafeSync` assertion | `conformance_in_the_traits_module_is_program_wide.saw`, `thread_assertion_survives_a_glob_import.saw` | 238c — was a DEVIATION: a conformance query walked the QUALIFIER bindings, which a GLOB deliberately never makes, so `import m.*` lost every conformance `m` declares for a type declared ELSEWHERE (the rule's second half, whose declaration site is the trait's module) and the diagnostic hinted at adding the extension that was already there. `coherence_search_namespaces` is the one funnel now, and its two entry points are `type_conforms_to` and `_lookup_thread_assertion` — the second is the row's other file, where a globbed `extension Shared: UnsafeSync {}` was invisible and a `static` of that type was refused as non-Sync. Name lookups are deliberately NOT routed through it: those are visibility questions, where a glob's copy already answers. DF-247b kept the claim true through the design 150 amendment: the SELECTIVE form stopped binding a qualifier too, so its source module moved out of `modules` and into `selective_sources`, which the same funnel now walks beside `glob_sources` — the qualifier was never what made a conformance visible |
| B24 | a QUALIFIER is bound only by the whole-module import form, and a file that writes the whole-module line beside a selective or glob one gets ONE TYPE from both spellings — annotation, construction, generic argument, `&any` existential, generic bound, struct field, enum payload, `type` alias, extension lookup and conformance coherence | `import247b_pair_one_identity.saw`, `import247b_glob_pair_one_identity.saw`, `import247b_nonbinding_form_qualifier_error.saw` | 247b — was a DEVIATION on both halves. The selective form's bonus qualifier reached everything its import list did NOT name, which is the undocumented dependency the braces exist to prevent; and under a glob the qualified spelling resolved to a NAME-ONLY type that compared unequal to the bare one (``cannot assign `Data` to variable of type `data.Data` ``) — a mismatch against a phantom, at a spelling design 150 promised worked in every position. The binding decision is one predicate now (`_import_binds_qualifier`, applied inside the one binding site), an unbound qualifier is a refusal naming the whole-module line that would bind it, and the newly-legal pair is pinned position by position because a fresh identity anywhere would be silent |
| B25 | a `public import` FORWARDS extension scope — one hop, a chain of facades, and every re-export form (whole-module, selective) — while a PLAIN import in the facade forwards nothing, the design-80 member gate still refuses a non-`public` forwarded method, and a collision is still the call-site ambiguity | `ext254_facade_forwards.saw`, `ext254_facade_chain.saw`, `ext254_facade_selective.saw`, `ext254_facade_overload_set.saw`, `ext254_plain_import_no_forward_error.saw`, `ext254_facade_visibility_error.saw`, `ext254_facade_collision_error.saw` | 254 — the fence on B06, and B06 is the fence on this: the hazard design 142 exists to stop is an UNRELATED transitive dependency changing what a call resolves to, and a `public import` is the facade author publishing that module deliberately. Widening scope alone was checked against the two rules that could have travelled with it and did not — name binding (nothing becomes reachable that was not) and the visibility tier |
| B26 | a std name an import GATE keeps hidden is the weakest bare name there is: a file's own DECLARATION of it wins, and so does an explicit IMPORT of it — at every form (`{N}`, `.*`, `{N as M}`) and every kind (generic struct, struct, enum) — while the qualified `task.Thread` still reaches std's in the same file. Both fences hold: a PRELUDE name stays reserved, and two EXPLICIT imports of one name stay ambiguous, at the USE span with both module labels | `shadow255_selective_import.saw`, `shadow255_local_declaration.saw`, `shadow255_glob_import.saw`, `shadow255_kinds_and_rename.saw`, `shadow255_qualified_escape.saw`, `shadow255_prelude_name_reserved_error.saw`, `shadow255_two_imports_ambiguous_error.saw` | 255 (sawos SL-5/SL-4) — was a DEVIATION, and its shape is why the row exists: ONE question ("whose declaration does this spelling name") had THREE answers. A declaration rebound the spelling; an import of a compiler-emitted std name tied and reported an ambiguity; an import of an ordinary gated one bound STD's symbol under the name with nothing said, failing later on a field or a variant the wrong type does not have. B12 is what bounds the ruling — the prelude core is reserved, not weak — and the remaining ambiguity report gained a real USE span and a real label per side, where it anchored at line 1 column 1 of an arbitrary file and called the merged side `<unknown>` |
| B27 | a BARE struct field takes the visibility OF ITS DECLARING TYPE, at every tier — `public` (read, written, and named in a cross-module memberwise literal), `public(package)` from a sibling, `public(parent)` refused from a non-parent | `vis258_field_inherits_public.saw`, `vis258_field_inherits_package.saw`, `vis258_field_inherits_parent_error.saw` | 258 — the ruling (user, Aug 31, after reading the sos code: "the member should inherit the visibility of their parent after all. lots of duplication the current way"). A `public(package) struct` no longer spells its tier on every field. The GATE is untouched — it only ever applied outside the defining module, and same-module access stays ungated; what moved is what an unmarked field's tier IS. The PRIVATE tier has no row of its own because it has no observable: private-tier inheritance IS module-private, so a private struct's bare fields mean exactly what they always meant, and the type is unreachable from outside anyway — which is why the overwhelming majority of the corpus compiled through the flip untouched. The `public(parent)` row lands on the TYPE for the neighbouring reason: a field can never be more reachable than the type it lives in |
| B28 | the boundary: FIELDS ONLY. An extension member — method, `static func`, `init` — inherits NOTHING from its receiver's tier, so a bare one is module-private on a `public struct` exactly as on a private one | `vis258_extension_members_do_not_inherit_error.saw` | 258 ruling 3 (user, over the lead's own-module-inheritance alternative): an extension method is a FUNCTION that happens to act on a struct, so it needs a DECLARED visibility or else it is a per-module helper; a field is the type's own data and rides the type's tier. The principle generalizes — nothing function-shaped inherits reach from a type — and two standing idioms are consequences that fall out and stand: the Aug-5 safe-utility-extension idiom (a bare helper on a foreign/std type is safe BECAUSE bare means private there) and the Aug-20 no-visibility-on-extension-heads ruling |
| B29 | `private` NARROWS a field back down, and is a CONTEXTUAL keyword: it is an ordinary identifier everywhere else (a local, a field NAME, a parameter label), and on any non-field declaration it is a clean error naming the default | `vis258_private_narrows_error.saw`, `vis258_private_is_an_ordinary_identifier.saw`, `vis258_private_on_a_function_error.saw` | 258 ruling 2 — the spelling that lets a visible type keep an internal field, which is what the whole in-tree migration is written in. Contextual on the spec's second-group terms (like `module`/`parent`), so `let private = 3` keeps compiling |
| B30 | the rule-5 interaction: a `public` struct's BARE field whose type is less visible is the design-193 refusal, and its hint names the third out — mark the field `private` | `vis258_inherited_field_needs_public_type_error.saw` | 258 ruling 5 — the shape was legal yesterday because the field was private, and inheriting the tier is what makes it a refusal. The rule itself is B20's and is unchanged; what changed is the reach it judges the field's type against, so the diagnostic owes the narrowing fix beside the widening one |
...
| O16 | at a declared `-> T?`, a bare `None` ARM of the tail's value branch is the OUTER absence and a VALUE arm owes exactly one wrap — per arm, at all four tail positions | `nested_optional_tail_branch_arms.saw` (concrete) + `optional_generic_vector.saw` / `optional_generic_map.saw` (instantiated) | DF-289d — design 176 made `?` nest, so `-> T?` at `T = Int?` is an ordinary signature and the two readings of a `None` arm stopped coinciding. The contextual-`None` funnel already stamps the arms on the outer reading; the wrap was then applied to the MERGED expression and put the layer in a second time, so `std/vector.saw`'s `pop` returned a three-layer value from a two-layer function. Concrete all along — the generic side only looked right because an abstract `V?` owes no wrap at all, and design 218 unit 1.5 stopped judging an instance abstractly |
...
| D21 | a `Never` that arrives by SUBSTITUTION is an ordinary value type — neither `void` + `noreturn` at the declaration nor `unreachable` after the call | `D21_substituted_never_is_a_value.saw` | DF-291a — `-> Never` is a claim about CONTROL FLOW that an instantiation never makes, which is design 132's ruling for `Void` at the return position. The carrier is design 141's place accessor: a `borrows` accessor is a method-generic over its window result `__R`, and a coroutine frame's dispatch infers `__R = Never` (every arm resumes or returns) while `Slot<T>.value` itself returns normally. Both spendings were suppressed by accident while a monomorphized body was built from the TEMPLATE under a live binding — the declared return read a type PARAMETER — and both fired the moment design 218 unit 1.5 stage 3c-2c made an instance an ordinary concrete declaration: SIGTRAP, no message |
...
| A04 | a refused `Vector(capacity:)` reports as a value — the fallible `init` in place, not a factory | `vector_capacity_reports_oom.saw` | 234 §1 + DF-245a — `Result<Self, E>` is the only fallible init form, so A02's rule is what this row consumes |
| A05 | `Vector.reserve` (the retired `try_reserve`, un-prefixed) reports, and a successful one makes the next pushes allocation-free | `alloc_vector_reports_oom.saw` | 234 §4 — one of the FIVE twins with no infallible partner, so retiring the prefix renames a sole method rather than merging a pair |
| A06 | a refused `Map.insert` reports; the table is untouched — every key that was in it still is | `alloc_map_set_reports_oom.saw` | 234 §1 — all-or-nothing, which the panic could never state because it never returned |
...
| A15 | BOUNDARY — `Data.[]`'s copy-on-write separation still panics, under the accessor rule it shares a body with | `alloc_data_reports_oom.saw` | 234 §5, ruled Aug 22 — splitting one of the two failures out as a value would make `d[i]` mean two different things by which failure it met; `detached()` is the preflight |
| A16 | BOUNDARY — a COLLECTION LITERAL cannot report, so its synthesized element insert forces the `Result` and panics NAMING the error | `alloc_collection_literal_panics_named.saw` | 234 §5 by analogy (flagged for ruling) — `[a, b, c]` lowers to a container plus synthesized `push`/`insert` calls, and a synthesized call is exactly a site with no expression to hang a `try` on. The row is also the negative control the teaching commit lacked: without it, the literal DROPPED the refusal silently and the suite stayed green |
...
| A18 | BOUNDARY — `Map` joins the ExplicitCopy tier (design 251) on Vector's own pattern, so `Map.copy()` panics on refusal exactly as A17 records for `Vector` | `alloc_map_copy_panics.saw` | 251 + 234 §5 + design 219 — same hook, same trait signature, same boundary; `Map.try_copy` is the reporting twin, mirroring `Vector.try_copy` verbatim (DF-257b's naming ruling still pending, unresolved by this brief) |
| A19 | BOUNDARY — `Set.copy()` panics too, delegating to `Map.copy()` (Set is a thin wrapper over `Map<T, SetMark, A>`), so the panic message names `Map.copy` rather than `Set.copy` | `alloc_set_copy_panics.saw` | 251 — the wrapper forwards its copy policy the same way it forwards `insert`'s `AllocError`; nothing about `Set` rewrites the message |
