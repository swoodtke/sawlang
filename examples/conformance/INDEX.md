...
| B27 | a BARE struct field takes the visibility OF ITS DECLARING TYPE, at every tier — `public`, `public(package)`, `public(parent)` and private — read, written and named in a cross-module memberwise literal | `vis258_field_inherits_public.saw`, `vis258_field_inherits_package.saw`, `vis258_field_inherits_parent_error.saw`, `vis258_field_inherits_private_error.saw` | 258 — the ruling (user, Aug 31, after reading the sos code: "the member should inherit the visibility of their parent after all. lots of duplication the current way"). A `public(package) struct` no longer spells its tier on every field. The GATE is untouched — it only ever applied outside the defining module, and same-module access stays ungated; what moved is what an unmarked field's tier IS |
