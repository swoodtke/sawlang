...
| E2 | core.py:752 `_unsafe_check_exempt` | design 130's unsafe trigger rule skips transformed bodies | resume bodies NAME `UnsafePointer` today (`__recv`, `ref` fields, `__cellp`, `__io_tok`), so every driven method's resume would owe an `unsafe` declaration | **DELETED, design 222 unit 4.** Narrowed at stage 3 as predicted, but the residue was NOT the cell/reactor plumbing this row guessed — design 222 unit 0 measured it and found 166 corpus files, all three constructs a CAST the transform spliced into a body somebody else wrote (spawn site 158, drive-site receiver 10, reference argument 2). `__cellp` and `__io_tok` never reached the flag: they are read only from declarations the transform AUTHORS, which say `unsafe` honestly under `unsafe_decl_checked`. Unit 2 moved the three crossings into those generated declarations (`&group` / `&c` at the call), which emptied the flag's coverage; unit 4 deleted the read site. The trusted list keeps the cell (item 2, narrowed) and the wake latch (item 3, with design 222 unit 3's traced argument) — as NAMED entries, which is not what an exemption is |
