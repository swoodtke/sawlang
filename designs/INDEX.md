...
- **242** — `242-thread-task-split.md` — the Thread/Task split: the namespace is the engine (`Thread.spawn -> Thread<T>`, `Task.spawn -> Task<T>` into a background singleton), must-consume handles with explicit join/detach/cancel, sync-enforced blocking-permitted thread bodies (nine rulings, Aug 22)
