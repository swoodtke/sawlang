...
- IMAGE SIZE (sos, Sep 2) — kernel images "very large"; NO size option exists today (`-O0`/O1 only). Already on: per-function sections + `--gc-sections` (168), zerofill statics, alloc-free format paths; freestanding hello .o is 3.2KB, so the growth is in reached code. MEASURE FIRST (doctrine): sos owes `llvm-size --format=sysv` + `nm --size-sort` tail + `--emit-bt-table` bytes off a real image; the remedy differs by section — `.text` → an `-Os`/`-Oz` pipeline flag (cheap; llvmlite supports it) + registry-driven instance dedup (the 218 registry can now COUNT identical-body instances); `.rodata` → a bt-table opt-out flag (trade: dump_tasks + panic backtraces) / panic strings; or a sos-side link.ld GC defeat. Brief when the numbers arrive
...
- DF-289d — a value BRANCH's merged type can disagree with its ARMS' stamped annotations, so an optional wrap decided from the merged type adds a layer the value already has (entry below, same filing; PRE-EXISTING, exposed by DF-286c face 4's fix). `Vector<Int?>.pop`'s `-> T?` at `T = Int?` returned a three-layer value from a two-layer function; a guard at the wrap predicate holds the line, and the disagreement itself is the finding
- DF-289e — **THE 3c-2c STOP**: the cutover's SECOND HALF (type-instance and method-generic bodies onto the ordinary `_generate_method`/`_generate_init_method` path) is a behavioural-contract flip that owes obligation 2's consumer sweep and its own dispatch (entry below, same filing). The free-function half LANDED and is green; the method half was built, measured against the corpus and REVERTED
...
## PRE-EXISTING, BLOCKS 3c-2c's second half)
...
FIX DIRECTION, probed and working on the reverted branch: the store holds a
LIST per (struct, name), the old readers take the first (what they always got),
and the materializer picks by owning extension and then by PARAMETER NAMES —
which is what distinguishes two inits and what the mangler itself keys an init's
symbol on. Four reader sites route through the accessors rather than the dicts
(`effects.py` ×4, `expressions.py`'s drive path, `coro_transform.py` ×2).
[218c stage 3c-2c]

## DF-289d — a value BRANCH's merged type can disagree with its ARMS' stamped
## annotations (filed Sep 2 by the same agent; PRE-EXISTING, exposed by
## DF-286c face 4's fix)
...
WHAT LANDED: `_branch_already_at`, a guard asked only where the payload-equality
clause would otherwise fire, which asks the ARMS instead of the merged type.
That holds the line and does not close the disagreement. The same family is why
`Map<String, Int?>`'s `_lend_var` window closure is still a shape mismatch under
3c-2c's second half.
[218c, DF-286c face 4]

## DF-289e — 3c-2c's SECOND HALF is a behavioural-contract flip and owes its own
## dispatch (filed Sep 2 by the same agent; THE STAGE STOP)
