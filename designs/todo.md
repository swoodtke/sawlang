...
- Design 250 — the `Byte` type (designs/250-byte-type.md; RULED IN PRINCIPLE Aug 27, user: "everything that uses bytes should use Byte" — SUPERSEDES 244's target, absorbs its census/funnels; OVERTURNS 245 §6's decline on probe evidence). Takes 244's queue slot. DISPATCH WAITS on the brief's §5 ruling sheet (annotated-let adoption, prelude placement, rt scope, Data mutation surface); the naming rider dissolves — `append(b: Byte)`/`index_of(b: Byte)` are sound overloads under a distinct type
