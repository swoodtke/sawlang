...
## at the DECLARATION HEAD) — **RULED Sep 4 (user): cell (b) + interim (c)**:
## design 207 is SCHEDULED (see its Sep-4 amendment, incl. the flagged
## annotation-driven cell SL-22's own shape demands) and the paren idiom is
## blessed in the spec/skill meanwhile (landed Sep 4). Closes when 207 lands

Probed (`.build/scratch/sl22_{break,paren}.saw`): the bare break is
`Parse error at 3:20: Unexpected token: NEWLINE`, anchored after the `=`;
**the PARENTHESIZED initializer compiles and runs today** —

    static WIDE: Pair = (
        Pair(a: 1, b: 2))

— design 129's bracket rule doing exactly what it says. Relayed to sos as
the immediate answer for the 100-118-char `Slab` heads.

WHY IT BITES: constructors do not infer type arguments (designs 93/105), so
a generic static writes `NAME: T<...> = T<...>(...)` — the type twice — on a
MANDATORY single line. THE RESOLUTION CELLS, all the user's: (a) amend the
grammar to allow a break after `=` (design 129 territory — the judgment "the
RHS is unambiguously incomplete" is new surface the bracket rule does not
currently make); (b) build design 207 (constructor type-arg inference through
the call solver — RULED to change Aug 10, unbuilt), which deletes the doubled
spelling and this pain with it; (c) bless + document the paren idiom and do
nothing else. Lead lean: (b) is the language the docs already promise, and
...
## DF-286c — the materialization funnel does not reproduce what codegen's
## `type_param_context` path did (filed Sep 1 by the same agent; ONE
## mechanism, four faces)

MECHANISM (obligation 4). Codegen has always built a generic body from the
TEMPLATE with a live `type_param_context`, so every lookup that needed a
binding asked the context at the moment it needed it — const parameter
values, associated types, bound-filtered method sets, return-position wrapping.
The cutover replaces that with a CLONE that must already carry every one of
those answers, and the copier substitutes `SawType`s and nothing else
(DF-285a was the first face of exactly this: a call NAME is a `str`). So the
mechanism is "an answer the live context supplied that the clone does not
carry", and the positions are enumerable.

The four found by materializing free functions and type-instance methods:

  1. A CONST-GENERIC parameter is a VALUE, not a type. `struct Ring<const N:
     Int>`'s methods clone to bodies reading `N` as an undefined variable, 54
     diagnostics over four examples, each cascading a "body has no value".
     `examples/generic_default_init.saw`, `const_generic_infer_length.saw`.
  2. An ASSOCIATED-TYPE annotation is not substituted in the clone's
     SIGNATURE. ``function `getItem$1$StringBox` should return `Item` but
     returns `String` `` — `_add_associated_type_bindings` puts `Item ->
     String` in the map and the return annotation keeps `Item`.
     `examples/associated_type_generic.saw`.
  3. The conditional-conformance BOUNDS FILTER is not applied to the method
     set the funnel materializes: `Map<String, Cell>.copy` is cloned and
     checked although `Cell` is not `ExplicitCopy`, so the clone calls
     `_key_ref`/`_value_ref` — methods that instantiation does not have —
     and cascades two `undefined variable`s. The registry's own
     `_bounds_satisfied` answers correctly; the materialization walk does not
     ask it for the ExplicitCopy-bounded extension.
     `examples/optional_generic_chain_assign_place_head.saw`.
  4. A `-> T?` TAIL's auto-wrap is missing on the clone. `wrap<Int?>` emits
     `ret {i1, i64}` from a `{i1, {i1, i64}}` function — invalid IR, so this
     one is an ICE rather than a diagnostic, and it is the face that says the
     clone is not merely judged differently but BUILT differently.
     `examples/generic_optional_tail_return.saw`.

Probed on a free-function-only slice of stage 3c-2 (materialize + splice fn
instances, `_instantiate_generic_function` replaced by a symbol lookup), which
was reverted: the branch carries 3c-0 and 3c-1 only. `test_runner.py -f
generic` under that slice: 147 passed, 9 failed, the nine being these four
faces plus DF-286b's classes 5 and 6.

Fix direction, not yet a plan: the funnel's clone has to be built from the
same answers the live context held, which means the const-parameter fold and
the associated-type substitution belong IN the copier (DF-285a's own lesson,
restated), the bounds filter belongs in the materialization walk beside the
registry's, and face 4 wants its own probe before anything is written. The
matrix above is stage 3c-2's test plan. [218c §1a, A2(a)]
