...
- Design 254 — extension scope follows `public import` (designs/254-extension-scope-public-import.md; user-approved + DISPATCHED Aug 30, one worktree with 255) — makes sawos design 8's ruled `Waiter.add(process:, key:)` writable behind the sos facade; ends with the shared `0.2.0` version bump (unit 3) so sawos can pin a release carrying both fixes; the sawos-side consumption is USER-OWNED
- Design 255 — explicit imports shadow the prelude + real ambiguity spans (designs/255-prelude-shadow-precedence.md; user-approved + DISPATCHED Aug 30, rides 254's worktree, runs second) — the sawos SL-4/SL-5 findings: `bind_type_name` first-wins vs `rebind_type_name` asymmetry, the hardcoded `1,1`/`<unknown>` report; unit 0 is the obligation-4 matrix sweep
