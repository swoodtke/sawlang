...
- Design 263 — panic scratch consolidation + narrow field reads (designs/263-panic-scratch-and-field-reads.md; DRAFT Sep 3, lead-authored from 261's refuting census — four rulings owed §3: L1 shape incl. the per-KIND-seam option, L2 same-brief-or-split, the load-side bool-renorm rider, queue slot). The successor to the IMAGE SIZE entry's levers; a back-end size lane (-Oz / machine outliner / riscv save-restore, plus a function-sections+gc-sections emission census cell) is the named NEXT tier after it, unauthored
