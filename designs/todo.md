...
- Design 265 — the back-end size lane (designs/265-backend-size-lane.md; SCHEDULED Sep 4 by the user — "next after 264": image size + perf pressure). U0 census FIRST (this family's diagnoses were measurement-refuted twice — 261(A), 263 U1/U3), U1 `-Os`/`-Oz` + machine outliner + riscv save-restore (the sure thing; default stays O1, two cells lead-recommended for dispatch confirmation), U2 = DF-300a's typed enum-construction stores GATED on U0's kill-criterion (≥5% of post-263 .text or park with numbers), U3 function-sections/gc-sections census report-only. Serial after 264 (both codegen); version bump at integration (user-visible flags)
